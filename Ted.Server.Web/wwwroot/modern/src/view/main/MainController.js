Ext.define('Admin.view.main.MainController', {
	...
	setCurrentView(hashTag) {
		...
		if (!item) {
			...
			if (hashTag.startsWith('ws:') && !vm.get('workspace')) {
				...
				AjaxUtil.get('/api/workspace/' + vm.get('user.token') + '/' + wsid, rsp => {
					...
					this.redirectTo(navData[0].routeId);
				});
			}
			else if (hashTag.startsWith('page:')) {

				let pageid = hashTag.split(':')[1];
				
				AjaxUtil.get('/api/component/' + vm.get('user.token') + '/' + pageid, rsp => {
				
					item = JSON.parse(rsp.data);

					item.xtype = item.xtype || 'workspace';
